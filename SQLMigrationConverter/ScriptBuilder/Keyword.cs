...
namespace SQLMigration.Converter.ScriptBuilder
{
    public class Keyword
    {
        ...
        public class SqlStatement
        {
            ...
            {
                var dtType = new PstScriptBuilder();
                ...
                var convertedDataType = dtType.GetDataTypeMap(dataType);     //Mapping.DataTypes.getRawMappingValue(dataType);
            ...
            void convertSelectClause()
            {
                ...
                if (_returnKind == enProcReturnKind.Resultset)
                {
                    ...
                    var query = Tools.getString(xml);
                    if (query.Trim().Substring(query.Trim().Length - 1, 1) != ";")
                        query = query + " ;";
                    var data = Tools.getXml(query)["SqlRoot"];
